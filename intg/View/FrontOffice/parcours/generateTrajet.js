...
function applyRoute() {
  ...
  if (typeof map !== 'undefined' && map && r.coords?.length >= 2) {
    ...
    Promise.all([
      ...
      // Use the shared updatePolyline() which draws the OSRM blue route
      if (typeof updatePolyline === 'function') {
        ...
        window.polyline = L.polyline(r.coords, {
          color:'#1d4ed8', weight:6, opacity:.92, lineJoin:'round'
        }).addTo(map);
...
function selectRoute(idx) {
  ...
  _gPoly.forEach((pl, i) => {
    if (!pl) return;
    pl.setStyle({ color: i===idx?'#1d4ed8':'#475569', weight: i===idx?6:3, opacity: i===idx?1:0.35 });
