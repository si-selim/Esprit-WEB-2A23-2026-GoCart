/**
 * parcours_map.js — v14
 * ✔ Type 1 marathon (1 région) : départ ET arrivée dans la même région
 * ✔ Type 2 marathon (2 régions) : départ dans région A, arrivée dans région B
 * ✔ Suggestions filtrées selon règle Type1/Type2
 * ✔ Validation : lieu hors région → ERREUR bloquante (rouge, pas simple avertissement)
 * ✔ Bouton "🧠 Recommander des trajets" → 6 trajets (2 facile / 2 moyen / 2 difficile)
 * ✔ Temps estimé en bas du panneau carte
 * ✔ Synchronisation bidirectionnelle formulaire ↔ carte
 */

const AI_PROXY = 'ai_proxy.php';   // Proxy PHP côté serveur — pas d'appel direct navigateur
const OSRM_API = 'https://router.project-osrm.org/route/v1/driving'; // OSRM routing gratuit
...
let markers = [];
let polyline = null;
let currentCity = '';  // ex: "Tunis" ou "Tozeur-Tataouine"
let _regionCenters = {}; // { 'Tunis': [lat,lng], 'Sousse': [lat,lng] }
let _iaRoutesCache = null;

// Cache suggestions IA
let _aiSuggestionsCache = { depart: null, arrivee: null };
let _aiSuggestionsLoading = { depart: false, arrivee: false };

/* ═══════════════════════════════════════
   BASE LOCALE DE LIEUX PAR VILLE
═══════════════════════════════════════ */
const CITY_PLACES = {
    'Ariana': [
...
/* ───────────────────────────────────────────
   ...
   Type 1 : 1 région  → départ + arrivée dans la même région
   Type 2 : 2 régions → départ dans région[0], arrivée dans région[1]
─────────────────────────────────────────── */
function getRegionParts() {
    return currentCity.split(/[-,]/).map(s => s.trim()).filter(Boolean);
}

function isMultiRegion() {
    return getRegionParts().length > 1;
}

/** Retourne les lieux valides pour le point de départ */
function getPlacesForDepart() {
    const parts = getRegionParts();
    // Type1 : tous les lieux de la région
    // Type2 : seulement la région A (index 0)
    const regionKey = parts[0];
    const key = Object.keys(CITY_PLACES).find(k => k.toLowerCase() === regionKey.toLowerCase());
    return key ? CITY_PLACES[key] : [];
}

/** Retourne les lieux valides pour le point d'arrivée */
function getPlacesForArrivee() {
    const parts = getRegionParts();
    // Retourner les lieux de TOUTES les régions du marathon (départ et arrivée libres)
    let allPlaces = [];
    parts.forEach(part => {
...
function getCityPlaces(cityName) {
    if (!cityName) return [];
...
function updateTimeDisplay(distKm) {
    ...
    if (distKm === null || distKm === undefined) {
        ...
        const tempsStr = tempsMin >= 60 ? `${Math.floor(tempsMin/60)}h ${tempsMin%60 > 0 ? tempsMin%60+'min' : ''}` : `${tempsMin} min`;
...
/* ═══════════════════════════════════════
   AUTOCOMPLETE — par type (depart/arrivee)
═══════════════════════════════════════ */
function initAutocomplete(inputId, type) {
    ...
    input.addEventListener('focus', () => {
        ...
        showLocalSuggestions(val, dropdown, input, type);
    });
...
/* ═══════════════════════════════════════
   SUGGESTIONS LOCALES FILTRÉES PAR REGION
═══════════════════════════════════════ */
...
function renderLocalSuggestions(places, query, dropdown, input, type) {
    ...
    if (!query || query.trim().length === 0) {
        ...
        renderSuggestionsHTML(places.slice(0, 8), '', dropdown, input, type);
        return;
    }
...
function renderSuggestionsHTML(places, query, dropdown, input, type) {
    const typeIcons = {
        'parc':'🌳','avenue':'🛣️','place':'🏛️','stade':'🏟️',
        'corniche':'🌊','medina':'🕌','monument':'🗿','autre':'📍'
    };
    const typeColors = {
    ...
    // Déterminer quelle région est affichée
    const parts = getRegionParts();
    const regionLabel = parts.join('-');
    const titleText = query
        ? `Lieux de ${regionLabel} · "${query}"`
        : `Lieux de ${regionLabel} — cliquez pour placer`;

    let html = `<div style="padding:10px 14px 6px;border-bottom:1px solid #f1f5f9;">
        <div style="display:flex;align-items:center;gap:7px;font-weight:800;color:#0f766e;font-size:.85rem;">
            📍 ${titleText}
        </div>
    ...
    places.forEach(lieu => {
        ...
        html += `
        <div class="ai-suggest-item" data-nom="${lieu.nom}" style="padding:10px 14px;cursor:pointer;border-bottom:1px solid #f8fafc;transition:background .15s;display:flex;align-items:flex-start;gap:10px;">
...
async function geocodeAndPlaceByName(name, type) {
    ...
    // Utiliser la bonne région selon type
    const parts = getRegionParts();
    ...
    // Fallback: centre de la région correspondante
...
async function reverseGeocode(latlng) {
    try {
        const r = await fetch(`https://nominatim.openstreetmap.org/reverse?lat=${latlng.lat}&lon=${latlng.lng}&format=json&accept-language=fr`);
        const d = await r.json();
        const a = d.address || {};
        const primaryName = a.amenity || a.tourism || a.leisure || a.historic || a.road || a.pedestrian || a.neighbourhood || a.suburb || a.village || a.town;
        const secondary = a.neighbourhood || a.suburb || a.district || a.quarter;
        if (primaryName && secondary && primaryName !== secondary) return `${primaryName}, ${secondary}`;
        if (primaryName) return primaryName;
        const area = a.quarter || a.district || a.city_district || a.suburb;
        if (area) return area + (a.city ? `, ${a.city}` : '');
        return a.city || currentCity;
    } catch(e){ return currentCity; }
}
...
async function placeMarkerFromInput(latlng, placeName, type) {
    ...
    else markers.push({ type, latlng: targetLatLng, marker });
...
async function onMapClick(e) {
    ...
    if (!cityBounds.contains(latlng)) {
        showMapToast('⚠️ Ce point est hors de la zone du marathon', 'warn');
        return;
    }
    ...
    if (!hasD) {
        ...
        await placeMarkerFromInput(latlng, name, 'depart');
        return;
    }
    if (!hasA) {
        const name = await reverseGeocode(latlng);
        ...
        await placeMarkerFromInput(latlng, name, 'arrivee');
        return;
    }
    await addWaypointBeforeArrivee(latlng);
    recomputeAll();
    updateMapInstructions();
}
...
async function addWaypointBeforeArrivee(latlng) {
    ...
    marker.on('dragend', () => { markers.forEach(m=>{ m.latlng=m.marker.getLatLng(); }); recomputeAll(); });
...
function recomputeAll() {
    ...
    const hasD = markers.some(m => m.type === 'depart');
    const hasA = markers.some(m => m.type === 'arrivee');
    if (!hasD || !hasA) { updateTimeDisplay(null); return; }
    ...
    if (diffEl) {
        ...
        // Fix: disabled select is not submitted — use a hidden input to carry the value
        ...
        if (!hidden) {
            ...
            diffEl.name = ''; // remove name from select so hidden wins
        }
        hidden.value = diff;
    }
...
function updatePolyline() {
    ...
    polyline = L.polyline(pts, { color:'#0f766e', weight:4, opacity:.9, dashArray:'8,4', lineJoin:'round' }).addTo(map);
    map.fitBounds(polyline.getBounds().pad(0.18));
}

function autoDifficulte(d) { return d<=10?'facile':d<=21?'moyen':'difficile'; }

/* ═══════════════════════════════════════
   LISTENER : saisie manuelle de distance
   → met à jour la difficulté en temps réel
═══════════════════════════════════════ */
(function() {
    function _applyAutoDiff() {
        ...
        // N'écraser que si le champ distance n'est pas verrouillé par un trajet généré
        if (distEl.readOnly) return;
        const diff = autoDifficulte(val);
        diffEl.value = diff;
        diffEl.className = diffEl.className.replace(/\bdiff-\S+/g, '').trim();
        diffEl.classList.add('diff-' + diff);
        // Sync hidden input si présent
...
/* ═══════════════════════════════════════
   HELPER : distance réelle via OSRM
═══════════════════════════════════════ */
async function getOSRMDistance(depLatLng, arrLatLng) {
    try {
        ...
        if (d.routes && d.routes[0]) {
            return d.routes[0].distance / 1000; // mètres → km
        }
    } catch(e) {}
    return null;
}
...
async function analyzeZonesAI(depLL, arrLL, city, distKm) {
    ...
    // Essayer d'obtenir la distance réelle via OSRM
    const osrmDist = await getOSRMDistance(depLL, arrLL);
    const finalDist = osrmDist || distKm;

    // Mettre à jour distance et difficulté avec la vraie valeur OSRM
    if (osrmDist) {
        const distEl = document.getElementById('distance');
        if (distEl) {
...
Maximum 4 zones. Utilise uniquement des lieux géographiques réels tunisiens.`;
    ...
    try {
        ...
        // Échec silencieux — pas d'erreur visible pour l'utilisateur
        panel.innerHTML = '';
        panel.style.display = 'none';
    }
}
...
function ensureRoutesModal() {
    ...
    #routesModalBody::-webkit-scrollbar { width:5px; }
    #routesModalBody::-webkit-scrollbar-thumb { background:#a7f3d0;border-radius:5px; }
...
{"routes":[{"id":1,"difficulty":"facile","depart":"Lieu réel","arrivee":"Lieu réel","description":"courte description"},{"id":2,"difficulty":"facile","depart":"...","arrivee":"...","description":"..."},{"id":3,"difficulty":"moyen","depart":"...","arrivee":"...","description":"..."},{"id":4,"difficulty":"moyen","depart":"...","arrivee":"...","description":"..."},{"id":5,"difficulty":"difficile","depart":"...","arrivee":"...","description":"..."},{"id":6,"difficulty":"difficile","depart":"...","arrivee":"...","description":"..."}]}`;
    ...
    try {
        ...
        // Geocode + OSRM pour vraies distances
        const routesWithDist = await Promise.all(routes.map(async route => {
            try {
                const regionDep = regionA;
                const regionArr = isMulti ? regionB : regionA;
                const [gDep, gArr] = await Promise.all([
                    fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(route.depart+', '+regionDep+', Tunisie')}&format=json&limit=1&countrycodes=tn`).then(r=>r.json()),
                    fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(route.arrivee+', '+regionArr+', Tunisie')}&format=json&limit=1&countrycodes=tn`).then(r=>r.json())
                ]);
                if (gDep[0] && gArr[0]) {
                    ...
                    return { ...route, distance_km: dist ? parseFloat(dist.toFixed(1)) : '—', _depLL: depLL, _arrLL: arrLL };
                }
            } catch(e) {}
        ...
        body.innerHTML = `<div style="padding:16px;background:#fef2f2;border-radius:12px;margin:8px;">
            ...
            <div style="font-size:.85rem;color:#64748b;">Configurez votre clé API dans <code>ai_proxy.php</code> (ligne 8).</div>
        </div>`;
    }
}
...
function renderRoutesModal(routes) {
    ...
    if (!routes || routes.length === 0) {
        body.innerHTML = `<div style="padding:20px;color:#b91c1c;">Aucun trajet généré. Réessayez.</div>`;
        return;
    }
    ...
    ['facile','moyen','difficile'].forEach(diff => {
        ...
        list.forEach((route, i) => {
            const routeData = JSON.stringify({ depart: route.depart, arrivee: route.arrivee, difficulty: route.difficulty, distance_km: route.distance_km }).replace(/"/g,'&quot;');
            const distLabel = route.distance_km !== '—' ? `📏 ${route.distance_km} km (OSRM)` : '📏 Distance à calculer';
...
function undoLastPoint() {
    ...
    if (markers.length >= 2) recomputeAll();
    else {
        unlockAutoFields();
        updateTimeDisplay(null);
        const p=document.getElementById('smart-panel'); if(p){p.style.display='none';p.innerHTML='';}
    }
    updateMapInstructions();
}
...
function resetMap() {
    ...
    const diffEl=document.getElementById('difficulte'); if(diffEl){ diffEl.value=''; diffEl.name='difficulte'; diffEl.className=diffEl.className.replace(/\bdiff-\S+/g,'').trim(); }
    const hiddenDiff=document.getElementById('difficulte_hidden'); if(hiddenDiff) hiddenDiff.parentNode.removeChild(hiddenDiff);
...
function updateMapInstructions() {
    ...
    const hasD = markers.some(m=>m.type==='depart'), hasA = markers.some(m=>m.type==='arrivee');
    const wpts = markers.filter(m=>m.type==='waypoint').length;
    const parts = getRegionParts();
    const isMulti = parts.length > 1;
    ...
    if (!hasD && !hasA) {
        ...
        el.innerHTML=`<span style="color:#475569;">✅ Tracé complet${wpts>0?' · <strong>'+wpts+' étape(s)</strong>':''} · <em>Cliquez sur la carte pour ajouter des points intermédiaires</em></span>`;
    }
}
...
function showMapToast(msg, type) {
    ...
    if(!t){t=document.createElement('div');t.id='map-toast';t.style.cssText='position:fixed;bottom:20px;left:50%;transform:translateX(-50%);padding:10px 20px;border-radius:12px;font-weight:700;font-size:.9rem;z-index:9999;transition:opacity .3s;';document.body.appendChild(t);}
    ...
    t.style.background=bg;t.style.color=color;t.style.border=`1px solid ${border}`;t.textContent=msg;t.style.opacity='1';
    clearTimeout(t._timer);t._timer=setTimeout(()=>{t.style.opacity='0';},3000);
}

/* ═══════════════════════════════════════
   FEEDBACK — VALIDATION PAR REGION
═══════════════════════════════════════ */
function triggerFeedback(id, val, type) {
    const el=document.getElementById(id); if(!el) return;
    if (!val) { el.textContent=''; el.className='feedback'; return; }

    // Pour distance / difficulte : pas de validation de région
    ...
    // Si le marqueur a été placé via la carte (clic, drag, ou suggestion choisie)
    // → le point est dans les bounds du marathon, toujours valide
    ...
    // Validation uniquement pour saisie manuelle (sans marqueur placé)
    // Refuser si c'est le nom d'un gouvernorat entier
    ...
    // Vérifier dans CITY_PLACES pour saisie manuelle
    ...
    if (validPlaces.length > 0) {
        ...
        const found = validPlaces.some(p =>
            ...
            v.includes(p.nom.toLowerCase())
        );
        if (!found) {
            const parts = getRegionParts();
            const regionLabel = parts.join('-');
            el.textContent = `❌ Ce lieu n'existe pas dans ${regionLabel}`;
            el.className = 'feedback error';
            return;
        }
    }
