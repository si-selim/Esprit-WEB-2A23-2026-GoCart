...
<body>
    ...
    if (isset($_GET['id'])) {
        $id = intval($_GET['id']);
        $sponsor = $controller->showSponsor($id);
    }
    ?>
    ...
    <div class="page">
        <div class="card">
            ...
            <p>Formulaire de modification pour un sponsor existant.</p>
            <?php if ($sponsor): ?>
            <form id="sponsorForm" action="modifySponsor_process.php?id=<?= $id ?>" method="post" enctype="multipart/form-data">
                <div class="grid">
                    <div class="field">
                        <label for="name">Nom</label>
                        <input id="name" name="name" type="text" placeholder="Nom du sponsor" value="<?= htmlspecialchars($sponsor['nom']) ?>">
                        <span id="name-error" class="error"></span>
                    </div>
                    <div class="field">
                        <label for="type">Type</label>
                        <select id="type" name="type">
                            <option value="Entreprise" <?= $sponsor['type'] == 'Entreprise' ? 'selected' : '' ?>>Entreprise</option>
                            <option value="Association" <?= $sponsor['type'] == 'Association' ? 'selected' : '' ?>>Association</option>
                            <option value="Particulier" <?= $sponsor['type'] == 'Particulier' ? 'selected' : '' ?>>Particulier</option>
                        </select>
                    </div>
                    <div class="field full-width">
                        <label for="address">Adresse</label>
                        <input id="address" name="address" type="text" placeholder="Adresse complète du sponsor" value="<?= htmlspecialchars($sponsor['adresse']) ?>">
                        <span id="address-error" class="error"></span>
                    </div>
                    <div class="field">
                        <label for="contact">Contact</label>
                        <input id="contact" name="contact" type="text" placeholder="0612345678" value="<?= htmlspecialchars($sponsor['contact']) ?>">
