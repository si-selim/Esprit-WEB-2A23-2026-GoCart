...
foreach ($marathons as $m) {
    if ($m['id_marathon'] == $data['id_marathon']) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $heure_depart = (isset($_POST['heure_depart']) && $_POST['heure_depart'] !== '') ? $_POST['heure_depart'] : null;
    $p = new Parcours(
        $id,
        $_POST['nom_parcours'] ?? '',
        $_POST['point_depart'] ?? '',
    ...
    if ($controller->modifierParcours($p, $id)) {
        ...
        header('Location: ../../FrontOffice/detailMarathon.php?id=' . $mid); exit;
    } else { $error = "Erreur lors de la modification du parcours."; }
}
...
<head>
    ...
    <style>
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .back-link{display:inline-flex;align-items:center;gap:8px;text-decoration:none;color:var(--teal);font-weight:700;margin-bottom:18px;padding:9px 16px;background:white;border-radius:12px;box-shadow:0 4px 12px rgba(16,42,67,.07);font-size:.95rem}
        ...
        .panel-header{display:flex;align-items:center;gap:12px;margin-bottom:16px;padding-bottom:16px;border-bottom:1px solid #e6edf3}
        ...
        input,select{width:100%;border-radius:11px;border:1px solid #cbd5e1;padding:10px 13px;font:inherit;outline:none!important;font-size:.97rem;background:#fff!important;color:var(--ink)!important;transition:border .15s;}
        ...
        *:focus{outline:none!important;}
        ...
        .btn{text-decoration:none;border:0;border-radius:11px;padding:12px 24px;font-weight:700;cursor:pointer;font-size:1rem;display:inline-flex;align-items:center;gap:7px;transition:transform .12s,box-shadow .12s,opacity .12s;}
        ...
        /* MAP PANEL */
        .map-panel-header{display:flex;align-items:center;gap:12px;margin-bottom:10px}
        ...
        .map-toolbar{display:flex;gap:8px;margin-bottom:8px;flex-wrap:wrap;align-items:center}
...
<div class="wrap">
    ...
    <div class="split-layout">
      ...
      <div class="panel">
        ...
        <p class="lead">Modifiez les champs ou retapez un lieu — les suggestions apparaissent automatiquement. La carte, la distance et la difficulté se mettent à jour automatiquement.</p>
        ...
        <form method="POST" novalidate>
          <div class="grid">
            <div class="field full">
              ...
              <?php
                $mNom='';
                foreach ($marathons as $m) {
                    if ($m['id_marathon']==$data['id_marathon']){$mNom=$m['nom_marathon'];break;}
                }
              ?>
              <div class="marathon-badge"><?php echo htmlspecialchars($data['id_marathon'].' - '.$mNom); ?></div>
            ...
            <div class="field">
              ...
              <input type="text" id="nom_parcours" name="nom_parcours" value="<?php echo htmlspecialchars($data['nom_parcours']); ?>" placeholder="Ex: Parcours Nord de Tunis">
              <div class="feedback" id="nomFeedback"></div>
            </div>
            <div class="field">
              <label for="heure_depart">Heure de Départ</label>
              <input type="time" id="heure_depart" name="heure_depart" required value="<?php echo htmlspecialchars($data['heure_depart']??''); ?>">
              <div class="feedback" id="heureFeedback"></div>
            </div>

            <div class="field">
              <label for="point_depart">📍 Point de Départ</label>
              <input type="text" id="point_depart" name="point_depart" value="<?php echo htmlspecialchars($data['point_depart']); ?>" placeholder="Tapez un lieu…" autocomplete="off">
              <div class="feedback" id="departFeedback"></div>
            </div>
            <div class="field">
              <label for="point_arrivee">🏁 Point d'Arrivée</label>
              <input type="text" id="point_arrivee" name="point_arrivee" value="<?php echo htmlspecialchars($data['point_arrivee']); ?>" placeholder="Tapez un lieu…" autocomplete="off">
            ...
            <div class="field">
              ...
              <select id="difficulte" name="difficulte">
                <?php foreach(['facile'=>'🟢 Facile','moyen'=>'🟡 Moyen','difficile'=>'🔴 Difficile'] as $v=>$lbl): ?>
                  <option value="<?php echo $v; ?>" <?php echo $data['difficulte']===$v?'selected':''; ?>><?php echo $lbl; ?></option>
                <?php endforeach; ?>
              </select>
          ...
          <!-- BOUTONS SAUVEGARDER / ANNULER EN BAS -->
      ...
      <div class="panel">
        <div class="map-panel-header">
          ...
          <div>
            ...
            <div style="font-size:.85rem;color:#64748b;margin-top:1px;">Les points s'affichent automatiquement — cliquez pour ajouter des étapes</div>
          </div>
        </div>
